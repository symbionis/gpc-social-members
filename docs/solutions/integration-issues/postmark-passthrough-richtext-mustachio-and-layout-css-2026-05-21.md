...
**Fix A — stop promising merge fields inside a passthrough body.** The composer (`components/admin/EventMessaging.tsx`) no longer advertises `{{first_name}}`/`{{email}}` as usable in the body; the hint now states the message sends as written. Greeting personalization was documented here as working via the template's own `{{#first_name}}…{{/first_name}}` block, which operates on the model field in the template source rather than on text the admin types into `body_html`. **Unresolved as of 2026-08-11:** no such block exists in the repo's copies of `docs/email-templates/event-message.html` or `docs/email-templates/members-comms-broadcast.html` (nor their `.txt` siblings), though the sender still passes the field and still names the block in a comment (`lib/broadcast/channels/email-transactional.ts`). Either the block lives only in the live Postmark template and was never mirrored into `docs/email-templates/`, or it was removed and the code comment is stale. Check the Postmark-side template before relying on greeting personalization in either channel. True per-recipient body personalization would require pre-rendering the body string per recipient before the send call; intentionally not done.
...
- This `{{{body_html}}}` passthrough pattern is shared with the **member broadcast** channel (`lib/broadcast/channels/email-postmark.ts` + the `members-comms-broadcast` template / `members-comms-layout`), so the merge-field limitation applies there too. If you wire personalization or change list/heading styling, update both layouts.
  ...
  **Not done, as of 2026-08-11.** `gpc-postmark-members-comms-layout.html` still carries the pre-fix state this doc describes: the old eyebrow `h3` (13px, weight 200, uppercase, letter-spaced) and no `ul`/`ol`/`li` rules at all. Only a `.rich-body p { margin: 0 }` rule was added, which addresses spacing but not indentation or the detached-bullet symptom. A rich-text broadcast to members still renders with the layout defects Fix B removed from the event-message layout.
