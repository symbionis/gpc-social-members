...
- [A registration-keyed door roster orphans legacy imported attendees](../architecture-patterns/registration-keyed-door-roster-orphans-imported-attendees.md) — the same registration-keyed reasoning, one surface earlier. There a registration-keyed projection made the person **invisible**; here a registration-keyed `Map.get` with a `?? "paid"` default made them **mislabeled**. Refreshed alongside this doc: it used to prescribe giving every comp guest a real zero-price registration, which KD10 reverses, and it now covers the door's two-builder architecture instead.
...
- [Cancelled seats reappear when counts derive from registration.quantity](cancelled-seats-reappear-when-counts-derive-from-registration-quantity.md) — the adjacent "a derived admin figure misrepresents guest-list seats" learning, and the other half of this bug's arithmetic: it now carries the corrected cancelled-count formula, `(paid + free) − active` rather than `booked − active`. Refreshed alongside this doc; its comp-era model statements are marked as history.
