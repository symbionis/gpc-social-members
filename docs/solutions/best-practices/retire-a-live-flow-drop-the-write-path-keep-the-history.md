...
- **Swept the types, missed the prose** (step 5, learned again in PRs #107–#111): the attendee CSV export route was deleted, but `lib/events/door-roster.ts` and `components/admin/ManageEventTabs.tsx` still described it as a live consumer (the door-roster header was corrected in PR #118; `ManageEventTabs.tsx:361` still says "same rows as the CSV"), and "Pre-registered" outlived its retirement in five comments across the door and roster code. A grep for the retired *noun* belongs in the same sweep as the grep for the retired path — a comment naming a deleted route is invisible to the type-checker for exactly the reason a runtime string endpoint is.
