-- Per-ticket-type capacity in the self-registration claim (U12).
--
-- A party buys a MIX of ticket types (e.g. 1 Without Asado, 3 Standard, 2 Veg, 2
-- Kids). The claim RPC already caps the party TOTAL, but a guest could pick a type
-- whose own allotment is already filled (e.g. the single "Without Asado" once one
-- guest has taken it). Enforce the per-type cap too: claimed-of-that-type may not
-- exceed purchased-of-that-type. Done under the existing registration row lock, so
-- the count is race-safe. A full type returns the new 'type_full' status (the page
-- also hides full types, so this is the concurrent-submit backstop).
--
-- Same 9-arg signature as 20260604160000 → CREATE OR REPLACE (no DROP needed).
...
BEGIN
  ...
  -- Per-type capacity: claimed of this type may not exceed purchased of this type.
  -- (Race-safe under the registration row lock taken above.)
