...
-- app code landing.
--
-- RESTORED TO MAIN 2026-08-15 (U8 of docs/plans/2026-08-15-001-feat-unified-purchase-module-plan.md):
-- this file previously existed only on branch feat/per-booking-ticket-limits (PR #135,
-- closed unmerged), but its migration was applied to the shared database regardless —
-- closing a PR does not unapply an already-run migration. The ledger
-- (supabase_migrations.schema_migrations) has carried version 20260814160000 /
-- event_booking_limits since it ran. This restoration is NOT a re-apply: the columns
-- already exist (verified live via information_schema.columns, 2026-08-15), this file is
-- only being reunited with the ledger entry that already names it, under its original
-- timestamp, so a fresh clone of main can reproduce the live schema.
