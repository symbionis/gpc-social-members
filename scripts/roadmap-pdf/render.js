...
// Poppins for the footer: header/footer templates are separate documents and cannot
// see the fonts embedded in the page, so the face is re-declared inline.
...
<style>
  @font-face{font-family:'Poppins';src:url(data:font/woff2;base64,${poppins}) format('woff2');font-weight:400;}
</style>
...
(async () => {
  ...
  // Cover: full bleed, no furniture.
  await page.goto(`file://${DOCS}/_roadmap_cover.html`, { waitUntil: 'networkidle' });
  await page.emulateMedia({ media: 'print' });
  await page.pdf({
    path: `${DOCS}/_cover.pdf`, format: 'A4', printBackground: true,
    margin: { top: 0, right: 0, bottom: 0, left: 0 },
  });

  // Body: margins, running band and footer.
  await page.goto(`file://${DOCS}/_roadmap_body.html`, { waitUntil: 'networkidle' });
  await page.emulateMedia({ media: 'print' });
  await page.pdf({
    path: `${DOCS}/_body.pdf`, format: 'A4', printBackground: true,
    displayHeaderFooter: true, headerTemplate, footerTemplate,
    margin: { top: '24mm', right: '26mm', bottom: '18mm', left: '26mm' },
  });

  await browser.close();
  console.log('rendered cover + body');
})();
