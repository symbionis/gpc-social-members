...
interface Props {
  ...
  baseUrl: string;
  /** Event id — used to build the staffed door-console link (/door/<id>). */
  eventId: string;
  arrivedCount: number;
...
// The "run the door" surface: a live arrivals count, the staffed door-console link +
// QR, and a recent-arrivals feed. All check-in now flows through the staffed console
// (FEAT-41 / U8 — the public self-service kiosk is retired): staff scan each guest's
// ticket QR there, or look them up by name. Mounted only while the Check-in tab is
// active, so the soft auto-refresh runs only when an organizer is watching.
export default function EventCheckInPanel({
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && !/^https?:\/\//.test(doorUrl)) {
      setDoorUrl(`${window.location.origin}${doorPath}`);
    }
  ...
  function downloadQr() {
    ...
    link.download = "door-console-qr.png";
    link.click();
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-xl border border-border bg-white p-5">
        <div className="flex items-baseline justify-between gap-3 mb-3">
          <p className="font-body text-sm text-muted-foreground">
            <span className="font-heading text-2xl font-bold text-marine">{arrivedCount}</span>{" "}
      ...
      <div className="grid gap-6 sm:grid-cols-[1fr_auto] sm:items-start">
        <section>
          ...
          <p className="font-body text-sm text-muted-foreground mb-3">
            Give this to whoever runs the desk — open it on a phone or tablet to scan
            each guest&apos;s ticket QR, look up a guest by name, fill in or edit
            details, and watch arrivals. No login. Print the QR for the desk.
          </p>
        ...
        <section className="flex flex-col items-center gap-2">
          <div ref={qrWrapRef} className="bg-white p-3 rounded-lg border border-border">
            <QRCodeCanvas value={doorUrl} size={140} marginSize={2} />
          </div>
          <button
            type="button"
            onClick={downloadQr}
      ...
      <section>
        <h4 className="font-heading text-base font-bold text-marine mb-3">Recent arrivals</h4>
        {arrivals.length === 0 ? (
