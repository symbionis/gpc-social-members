...
describe("GET /api/admin/events/[id]/attendees (CSV)", () => {
  ...
  it("exports the roster columns from event_attendees", async () => {
    ...
    expect(header).toBe(
      "name,email,phone,is_member,party_lead,waiver,arrived,arrived_at"
    );
    // Lead: member, signed waiver, arrived. The E.164 phone's leading + is
    // neutralized by the formula-injection guard (leading ').
    expect(rows[0]).toBe(
      "Ann Lead,ann@x.com,'+41781234567,yes,lead,signed,yes,2026-06-06T10:00:00Z"
    );
    // Guest attributed to the lead; phone-only/email handling; unsigned; not arrived.
    expect(rows[1]).toBe(
  ...
  it("renders a phone-only attendee without dropping it", async () => {
    ...
    // E.164 phone's leading + is quote-neutralized; the row is not dropped.
